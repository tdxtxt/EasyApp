...
import comm.helper.Logger;
import comm.helper.NetworkHelper;
...
public class OkHttpProvider {
    ...
    private static OkHttpClient getOkHttpClient(Interceptor cacheControl) {
        ...
        //设置缓存
        File httpCacheDirectory = new File(EasyApp.getInstance().getCacheDir(), "OkHttpCache");
...
//        httpClientBuilder.addInterceptor(new UserAgentInterceptor("Android Device"));
    ...
    private static class CacheControlInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetworkHelper.isConnected(EasyApp.getInstance())) {
...
//    }
    ...
    private static class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            Logger.log(String.format(Locale.CHINA, "Sending request %s on %s%n%s",
                    request.url(), chain.connection(), request.headers()));

            Response response = chain.proceed(request);

            long t2 = System.nanoTime();
            Logger.log(String.format(Locale.CHINA, "Received response for %s in %.1fms%n%s",
